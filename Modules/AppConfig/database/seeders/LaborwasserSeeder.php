...
/**
 * Labor Wasser de Mexico — tenant-specific AppSettings seeder.
 *
 * This is the LWM override of the generic AppSettingSeeder placeholder
 * values. Source data comes from the client's official datasheet (Bind ERP,
 * 2026-03-16): company name, phones (55 7575 1661 main), sales email
 * (ventas@laborwasserdemexico.com), web (laborwasser.com), SMTP config
 * (mail.laborwasserdemexico.com), branding colors (#8AC905 lime green),
 * and currency configuration (MXN base, Banxico rates).
 *
 * Idempotency: every entry uses firstOrCreate so re-running this seeder
 * never overwrites edits the admin has made through the dashboard at
 * /dashboard/settings/app-config. To force a reset of a single key, run:
 *   AppSetting::where('key', 'company.name')->delete();
 *   php artisan db:seed --class=LaborwasserSeeder
 *
 * Expected to run AFTER CleanDatabaseSeeder in fresh deploys.
 */
class LaborwasserSeeder extends Seeder
{
    public function run(): void
    {
        $settings = [
            // ========================================
            // Company identity (from Bind ERP 2026-03-16)
            // ========================================
            ['key' => 'company.name', 'value' => 'Labor Wasser de Mexico', 'type' => 'string', 'group' => 'company', 'label' => 'Nombre de la empresa'],
            ['key' => 'company.legal_name', 'value' => 'LABOR WASSER DE MEXICO S.A DE C.V.', 'type' => 'string', 'group' => 'company', 'label' => 'Razon social (fiscal)'],
            ['key' => 'company.rfc', 'value' => 'LWM2005274S2', 'type' => 'string', 'group' => 'company', 'label' => 'RFC'],
            ...
            ['key' => 'company.country', 'value' => 'Mexico', 'type' => 'string', 'group' => 'company', 'label' => 'Pais'],
            ...
            // ========================================
            // Branding palette (LWM lime green)
            // ========================================
            ['key' => 'branding.primary_color', 'value' => '#8AC905', 'type' => 'string', 'group' => 'branding', 'label' => 'Color primario (lime green)'],
            ...
            // ========================================
            // Mail / Email del sistema (via mail.laborwasserdemexico.com)
            // ========================================
            ...
            // ========================================
            // Currency (MXN base with Banxico auto-update)
            // ========================================
