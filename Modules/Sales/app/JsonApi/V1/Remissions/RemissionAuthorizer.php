...
/**
 * SA-M006: Remission Authorizer
 */
class RemissionAuthorizer implements Authorizer
{
    /**
    ...
    {
        $user = $request->user();
        return $user?->can('remissions.index') ?? false;
    }
    ...
    {
        $user = $request->user();
        return $user?->can('remissions.store') ?? false;
    }
    ...
    {
        $user = $request->user();
        return $user?->can('remissions.show') ?? false;
    }
    ...
    {
        $user = $request->user();
        return $user?->can('remissions.update') ?? false;
    }
    ...
    {
        $user = $request->user();
        return $user?->can('remissions.destroy') ?? false;
    }
    ...
    {
        return $request->user()?->can('remissions.show') ?? false;
    }
    ...
    {
        return $request->user()?->can('remissions.update') ?? false;
    }
