...
class ExchangeRateIndexTest extends TestCase
{
    ...
    public function test_admin_can_sort_ExchangeRates_by_baseCurrency(): void
    {
        $admin = $this->getAdminUser();
        
        ExchangeRate::factory()->create(['base_currency' => 'test string']);
        ExchangeRate::factory()->create(['base_currency' => 'test string']);

        $response = $this->actingAs($admin, 'sanctum')
            ->jsonApi()
            ->expects('exchange-rates')
            ->get('/api/v1/exchange-rates?sort=baseCurrency');
